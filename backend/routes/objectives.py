        ...
        try:
            ...
            with main_app.get_db() as conn:
                ...
                conn.execute(
                    ...
                    (data['key'], data['area_key'], data['text'], main_app.get_pacific_time(), 'open', next_order)
                )
                conn.commit()
    ...
    if request.method in ['PUT', 'PATCH']:
        try:
            ...
            with main_app.get_db() as conn:
                ...
                if current:
                    main_app.log_action_for_undo(conn, 'UPDATE', 'objectives', key, dict(current))
                ...
                if 'status' in data:
                    ...
                    if data['status'] == 'complete':
                        ...
                        conn.execute(
                            'UPDATE tasks SET status = "complete", date_time_completed = ? WHERE objective_key = ? AND status != "complete"',
                            (main_app.get_pacific_time(), key)
                        )
                    else:
                        updates.append('date_time_completed = ?')
                        values.append(None)
        ...
        try:
            with main_app.get_db() as conn:
                ...
                if objective:
                    ...
                    main_app.log_action_for_undo(conn, 'DELETE', 'objectives', key, dict(objective))
